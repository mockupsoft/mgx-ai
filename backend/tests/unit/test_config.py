...
class TestTaskMetricsValidators:
    """Test TeamConfig validators by calling them directly to improve coverage."""
    
    def test_task_metrics_max_rounds_validator_invalid(self):
        """Test max_rounds validator with invalid value by calling directly."""
        from mgx_agent.roles import TeamConfig
        
        # Call validator directly to cover satır 121
        with pytest.raises(ValueError, match="max_rounds en az 1 olmalı"):
            TeamConfig.validate_max_rounds(0)
        
        with pytest.raises(ValueError, match="max_rounds en az 1 olmalı"):
            TeamConfig.validate_max_rounds(-1)
    
    def test_task_metrics_investment_validator_invalid(self):
        """Test investment validator with invalid value by calling directly."""
        from mgx_agent.roles import TeamConfig
        
        # Call validator directly to cover satır 127-129
        with pytest.raises(ValueError, match="investment en az \\$0.5 olmalı"):
            TeamConfig.validate_investment(0.4)
        
        with pytest.raises(ValueError, match="investment en az \\$0.5 olmalı"):
            TeamConfig.validate_investment(0.0)
    
    def test_task_metrics_budget_multiplier_zero(self):
        """Test budget_multiplier validator with zero/negative by calling directly."""
        from mgx_agent.roles import TeamConfig
        
        # Call validator directly to cover satır 134-135
        with pytest.raises(ValueError, match="budget_multiplier 0'dan büyük olmalı"):
            TeamConfig.validate_budget_multiplier(0)
        
        with pytest.raises(ValueError, match="budget_multiplier 0'dan büyük olmalı"):
            TeamConfig.validate_budget_multiplier(-1)
    
    def test_task_metrics_budget_multiplier_high_warning(self, caplog):
        """Test budget_multiplier validator warning by calling directly."""
        from mgx_agent.roles import TeamConfig
        import logging
        
        # Call validator directly to cover satır 136-137
        with caplog.at_level(logging.WARNING):
            result = TeamConfig.validate_budget_multiplier(11)
...
class TestTeamConfigValidatorDirectCalls:
    """Test TeamConfig validators by calling them directly to improve coverage."""
    
    def test_validate_max_rounds_direct_call(self):
        """Test validate_max_rounds validator by calling it directly."""
        from mgx_agent.roles import TeamConfig
        
        # Valid value should return the value
        result = TeamConfig.validate_max_rounds(5)
        assert result == 5
    ...
    def test_validate_investment_direct_call(self):
        """Test validate_investment validator by calling it directly."""
        from mgx_agent.roles import TeamConfig
        
        # Valid value should return the value
        result = TeamConfig.validate_investment(1.0)
        assert result == 1.0
        
        # Boundary value should work
        result = TeamConfig.validate_investment(0.5)
        assert result == 0.5
    ...
    def test_validate_budget_multiplier_direct_call_zero(self):
        """Test validate_budget_multiplier validator with zero/negative by calling directly."""
        from mgx_agent.roles import TeamConfig
        
        # Valid value should return the value
        result = TeamConfig.validate_budget_multiplier(1.0)
        assert result == 1.0
        
        # Invalid value should raise ValueError
        with pytest.raises(ValueError, match="budget_multiplier 0'dan büyük olmalı"):
            TeamConfig.validate_budget_multiplier(0.0)
        
        with pytest.raises(ValueError, match="budget_multiplier 0'dan büyük olmalı"):
            TeamConfig.validate_budget_multiplier(-1.0)
    
    def test_validate_budget_multiplier_direct_call_warning(self, caplog):
        """Test validate_budget_multiplier validator warning by calling directly."""
        from mgx_agent.roles import TeamConfig
        import logging
        
        # High value should trigger warning but return the value
        with caplog.at_level(logging.WARNING):
            result = TeamConfig.validate_budget_multiplier(11.0)
            assert result == 11.0
            # Check that warning was logged
            assert "budget_multiplier çok yüksek" in caplog.text or "Maliyet patlaması riski" in caplog.text
        
        # Value at threshold should not trigger warning
        result = TeamConfig.validate_budget_multiplier(10.0)
        assert result == 10.0


class TestTeamConfigHelperMethodEdgeCases:
    """Test TeamConfig helper methods with edge cases for better coverage."""
    
    def test_to_dict_comprehensive(self):
        """Test to_dict() with all fields populated."""
        from mgx_agent.roles import TeamConfig, LogLevel
        
        config = TeamConfig(
            max_rounds=10,
            max_revision_rounds=3,
            max_memory_size=500,  # Max allowed value
            enable_caching=True,
            cache_backend="redis",
        ...
        assert result['max_revision_rounds'] == 3
        assert result['max_memory_size'] == 500
        assert result['enable_caching'] is True
        assert result['cache_backend'] == "redis"
        assert result['cache_max_entries'] == 200
        assert result['cache_ttl_seconds'] == 7200
        assert result['human_reviewer'] is True
        assert result['default_investment'] == 5.0
        assert result['budget_multiplier'] == 2.0
        assert result['log_level'] == LogLevel.DEBUG
        assert result['enable_metrics'] is True
        assert result['enable_progress_bar'] is True
        assert result['enable_streaming'] is True
        assert result['auto_approve_plan'] is True
        assert result['cache_log_hits'] is True
        assert result['cache_log_misses'] is True
    ...
    def test_from_dict_with_missing_fields(self):
        ...
        # Should have max_rounds
        assert config.max_rounds == 5
        # Should have default values for other fields
        assert config.max_revision_rounds == 2  # Default
        assert config.enable_caching is True  # Default
        assert config.human_reviewer is False  # Default
    ...
    def test_from_yaml_file_not_found(self, tmp_path):
        ...
        with pytest.raises(FileNotFoundError):
            TeamConfig.from_yaml(str(non_existent_file))
    
    def test_from_yaml_invalid_yaml(self, tmp_path):
        """Test from_yaml() with invalid YAML format."""
        from mgx_agent.roles import TeamConfig
        ...
        with pytest.raises(Exception):  # yaml.YAMLError or similar
            TeamConfig.from_yaml(str(invalid_yaml_file))
    
    def test_save_yaml_permission_error(self, tmp_path):
        """Test save_yaml() with valid path (permission errors hard to simulate on Windows)."""
        from mgx_agent.roles import TeamConfig
        
        config = TeamConfig(max_rounds=5, default_investment=3.0)
        ...
        assert "5" in content
        assert "default_investment" in content
    ...
    def test_str_representation_all_fields(self):
        ...
        assert "TeamConfig" in str_repr
        assert "max_rounds=15" in str_repr
        assert "max_revision_rounds=5" in str_repr
        assert "max_memory_size=500" in str_repr
        assert "enable_caching=True" in str_repr
        assert "human_reviewer=True" in str_repr
        assert "default_investment=$10.0" in str_repr
